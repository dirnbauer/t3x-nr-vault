# ADR 0004: If Rust transport is used via PHP FFI, production mode is “preload-only” (no dynamic FFI in requests)
...
- Enable full FFI at runtime (`ffi.enable=true`).  
  - Rejected: unacceptable risk in typical web hosting setups.
- Use `ext-php-rs` or a custom PHP extension.  
